...
const StatisticModal = ({ show, onClose, onSubmit }) => {
    ...
    useEffect(() => {
        ...
        fetchData();
    }, [selectedFees, feeTypeList, date.month, date.year]);
    ...
    const handleNextStep = () => {
        ...
        if (!/^\d{4}$/.test(date.year)) {
            toast.error("Số năm không hợp lệ!");
            return;
        }
        if (!/^\d{1,2}$/.test(date.month)) {
    ...
    return (
        <Modal size={step === 2 ? 'xl' : 'md'} className='modal-create' show={show} onHide={onClose} centered>
            ...
            <Modal.Footer>
                ...
                {step === 1 && <Button className='savebtn' variant="primary" onClick={handleNextStep}>Tiếp tục</Button>}
                {step === 2 && <Button className='savebtn' variant="success" onClick={handleSubmit}>Xuất file</Button>}
            </Modal.Footer>
        </Modal>
    );
};
